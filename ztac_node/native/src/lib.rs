...
extern crate ztac_circuit;
extern crate ztac_primitives;
extern crate phase2;
extern crate rand;
...
use ztac_circuit::circuit::{Transfer, MERKLE_PROOF_LEN, UtxoAccumulator};
use ztac_primitives::transactions::NoteData;
use ztac_primitives::fieldtools::fr_to_repr_bool;
use ztac_primitives::serialization::read_fr_repr_be;
use ztac_primitives::verifier;
...
pub fn note_hash(mut cx: FunctionContext) ->JsResult<JsBuffer> {
    ...
    let hash = ztac_primitives::transactions::note_hash(&note, &JUBJUB_PARAMS);
...
pub fn nullifier(mut cx: FunctionContext) ->JsResult<JsBuffer> {
    ...
    let nf = ztac_primitives::transactions::nullifier::<Bls12>(&note_hash, &sk, &JUBJUB_PARAMS);
    fr_to_js(&mut cx, &nf)
}
...
pub fn pubkey(mut cx: FunctionContext) ->JsResult<JsBuffer> {
    ...
    let sk = read_buf_fr(&mut cx, sk)?;
    let nf = ztac_primitives::transactions::pubkey::<Bls12>(&sk, &JUBJUB_PARAMS);
    fr_to_js(&mut cx, &nf)
}
...
pub fn edh(mut cx: FunctionContext) ->JsResult<JsBuffer> {
    ...
    let sk : Handle<JsBuffer> = cx.argument(1)?;
    let sk = read_buf_fr(&mut cx, sk)?;
    let nf = ztac_primitives::transactions::edh::<Bls12>(&pk, &sk, &JUBJUB_PARAMS).ok_or(()).or_else(|_| cx.throw_error("Not an elliptic curve point"))?;
    fr_to_js(&mut cx, &nf)
}
...
pub fn merkle_hash(mut cx: FunctionContext) -> JsResult<JsBuffer> {
    ...
    let hash = ztac_primitives::pedersen_hasher::compress::<Bls12>(&left, &right, Personalization::MerkleTree(n.round() as usize), &JUBJUB_PARAMS);
    fr_to_js(&mut cx, &hash)
}
